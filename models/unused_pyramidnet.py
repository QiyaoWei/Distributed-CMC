...
class PyramidNet(nn.Module):
    def __init__(self, inp, num_layers, alpha, block, num_classes=128):
        ...
        self.conv1 = nn.Conv2d(in_channels=inp, out_channels=16, kernel_size=3,
                               stride=1, padding=1, bias=False)
        ...
        #self.temp = nn.Linear(magic, self.out_channels)
        magic = 13279
        self.fc_out = nn.Linear(magic, num_classes)
...
class pyramidnet(nn.Module):
    def __init__(self, name='pyramidnet'):
        super(pyramidnet, self).__init__()
        block = ResidualBlock
        self.l_to_ab = PyramidNet(inp=1, num_layers=18, alpha=270, block=block)
        self.ab_to_l = PyramidNet(inp=2, num_layers=18, alpha=270, block=block)
    def forward(self, x):
