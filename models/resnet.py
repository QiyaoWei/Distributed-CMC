...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layerss = []
        layerss.append(block(self.inplanes, planes, stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layerss.append(block(self.inplanes, planes))

        return nn.Sequential(*layerss)
...
class MyResNetsCMC(nn.Module):
    def __init__(self, name='resnet50v1'):
        ...
        #Qiyao commented this line, otherwise DDP will not work
        #self.encoder = nn.DataParallel(self.encoder)
